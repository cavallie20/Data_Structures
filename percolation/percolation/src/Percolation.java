...
public class Percolation {
	...
	private int         virtualTop;    // virtual top    index on WeightedQu ckUnionFind arrays
	...
	*/
	public void openSite (int row, int col) {
		...
		grid[row][col] = true;
		int x = (row) * gridSize + (col);
		
		if(row == 0){
			wquFind.union(x, virtualTop);
		}

		if(row == gridSize-1){
			wquFind.union(x, virtualBottom);
		}

		if(row >= 1 && (grid[row - 1][col] == true) ){
			wquFind.union(x, (row - 1) * gridSize + col);
		}
		...
		if(col >= 1 && (grid[row][col - 1] == true) ){
			wquFind.union(x, (row) * gridSize + (col-1));
		}
